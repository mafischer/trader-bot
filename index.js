...
async function main() {
  ...
  await new Promise((resolve, reject) => {
    robinhood.investment_profile((err, response, body) => {
      if (err) {
        return reject(err);
      }
  ...
  while (!exit) {
    ...
    await eachSeries(following, async (tweeter) => {
      ...
      if (Object.prototype.hasOwnProperty.call(tweets, 'data') && Array.isArray(tweets.data)) {
          ...
          if (typeof tweet.text === 'string') {
            ...
            do {
              ...
              if (match) {
                ...
                try {
                  await new Promise((resolve, reject) => {
                    robinhood.quote_data(stock, (err, response, body) => {
                      if (err) {
                        reject(err);
                      } else {
                        console.log(`\nquote for ${stock}:\n${JSON.stringify(body)}\n`);
